#!/usr/bin/env python3
"""
Тестовая версия рефакторенного бота для проверки функциональности
"""
...
async def test_session():
    ...
    # Тестируем состояние
    session.current_dish = "паста карбонара"
    session.current_weight = 250
    session.current_cooking_method = "варка"
    
    print(f"   Блюдо: {session.current_dish}")
    print(f"   Вес: {session.current_weight}г")
    print(f"   Способ: {session.current_cooking_method}")
    
    # Тестируем сброс
    session.reset_dish_state()
    print(f"   После сброса - блюдо: {session.current_dish}")
